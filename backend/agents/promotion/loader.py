...
from backend.agents.promotion.models import PromotionConfig

logger = logging.getLogger("backend.agents.promotion.loader")
...
class PromotionLoadError(ValueError):
    """Canonical input cannot be used for the Promotion Agent."""
...
def refuse_non_integrated(path: Path) -> None:
    ...
    if "raw" in parts and "integrated" not in parts:
        raise PromotionLoadError(
            "Promotion Agent V1 consumes only the canonical integrated dataset, not data/raw/"
        )
    if name.endswith(".clean.csv"):
        raise PromotionLoadError(
            "Promotion Agent V1 consumes the canonical integrated commercial CSV, not Data QA *.clean.csv files"
        )
...
def discover_integrated_files(path: Path) -> list[Path]:
    ...
    if path.is_dir():
        ...
        if not files:
            raise PromotionLoadError(f"No *.commercial.csv files found in {path}")
        return files
    if not path.is_file():
        raise PromotionLoadError(f"Integrated input does not exist: {path}")
...
def select_integrated_file(files: list[Path], manufacturer: str) -> tuple[Path, pd.DataFrame]:
    ...
    if not usable:
        raise PromotionLoadError(
            ...
            "with product, retailer, region, date, sales, and stores"
        )
...
def load_integrated_unilever(path: Path, config: PromotionConfig) -> tuple[Path, pd.DataFrame]:
    ...
    logger.info("promotion_loaded manufacturer=%s in_pos_rows=%s", config.manufacturer, len(subset))
