/**
 * Keep only stories that can affect Unilever Home Care categories:
 * laundry detergent, laundry bars, dishwashing, toilet care,
...
const SOUTH_AFRICA =
  /\b(south africa|south african|gauteng|kwazulu-natal|\bkzn\b|western cape|shoprite|checkers|usave|pick n pay)\b/i;
...
function hasSaCategoryNews(text: string): boolean {
  return CATEGORY_TERMS.test(text) && SOUTH_AFRICA.test(text);
}

export function isHomeCareRelevant(title: string, summary = ""): boolean {
  ...
  if (hasNamedHomeCareBrand(text)) return true;
  if (hasUnileverHomeCare(text)) return true;
  if (hasSaCategoryNews(text)) return true;
  return false;
}
...
export function signalIsHomeCareRelevant(signal: {
  ...
  return isHomeCareRelevant(signal.title, signal.summary ?? "");
}
